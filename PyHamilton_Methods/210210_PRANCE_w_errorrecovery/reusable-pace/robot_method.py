...
def pump_culture(input_dict):
    ...
    if bacteria_id not in bacteria_controller.values():
        ...
        culture_total_vol=sum([1 for well in controller_list if well==bacteria_id])*culture_vol
...
def reader_plate_prep(input_tuple):
    ...
    #    return (reader_plate, reader_plate_rotation, reader_plate_slice, 
    #            lagoon_tips, lagoon_tips_rotation, lagoon_tips_slice, 
    #            iteration_counter, iter_start_time, 
    #            error_recovery)
    ...
    if rplate_start == 0: # if we're starting (over) at index 0 of a reader plate
    ...
    for bacteria in bacteria_id_tuple:
        ...
        for column in range(0, num_lagoons//8*2, 2): # explicitly skip every other column in this method by supplying 2 as range()'s 3rd arg
            ...
            for i in range(8):
                ...
                if well_id in bacteria_to_use and bacteria_to_use[well_id] == bacteria:
                    column_tips.append((lagoon_tips, num))
                    column_vols.append(350) #TODO: actual replacement volume?
