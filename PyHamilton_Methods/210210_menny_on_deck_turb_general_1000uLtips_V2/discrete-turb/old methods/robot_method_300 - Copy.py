...
def ensure_transfer_table_exists(db_conn):
    ...
    c = db_conn.cursor()
    c.execute('''CREATE TABLE if not exists transfer_table_v3
                (flowrates, k_estimates, od_estimates, replacement_vols, plate_keys, well_ids, timestamp)''')
    db_conn.commit()
...
def db_add_transfer_data(flowrates,k_estimates,od_estimates,replacement_vols,plate_key,well_id,timestamp):
    ...
    c.execute('INSERT INTO transfer_table_v3 VALUES (?,?,?,?,?,?,?)', data) 
    while True:
        try:
        ...
        try:
            ...
            while True:
                for turbs_for_plate, controllers_for_plate, plate, tips, media_reservoir in zip(turbs_by_plate, ctrlrs_by_plate, plates, tip_boxes, media_sites):
                    ...
                    if not os.path.exists(os.path.join(method_local_dir, containing_dirname + '.db')):
                        platedatas = [PlateData(os.path.join('assets', 'dummy_platedata.csv'))]*len(reader_protocols) # sim dummies
