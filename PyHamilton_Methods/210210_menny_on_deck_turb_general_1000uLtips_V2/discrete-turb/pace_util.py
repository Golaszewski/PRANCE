...
LAYFILE = os.path.join(this_file_dir, 'assets', 'deck1000uL.lay')
...
from pyhamilton import (HamiltonInterface, LayoutManager, ResourceType, Plate24, Plate96, Tip96,
    ...
    oemerr, PositionError, HamiltonReturnParseError, IllegalTargetPlatePositionError)
...
def tip_pick_up(ham_int, pos_tuples, **more_options):
    ...
    for _ in range(10):
        try:
            response=ham_int.wait_on_response(ham_int.send_command(PICKUP,
                labwarePositions=labware_poss,
                channelVariable=ch_patt,
                **more_options), raise_first_exception=True)
            logging.info("aspirate 96 response")
            logging.info(response)
            print(response)
            break
        except HamiltonReturnParseError:
            time.sleep(60*3)
            logging.info("Encountered error, trying again")
            pass
...
default_liq_class = 'HighVolumeFilter_Water_DispenseSurface_Empty'
    ...
    for _ in range(10):
        try:
            ham_int.wait_on_response(ham_int.send_command(PICKUP96,
                ...
                **more_options), raise_first_exception=True)
            break
        except IllegalTargetPlatePositionError:
            time.sleep(60*3)
            pass
...
def aspirate_96(ham_int, plate96, vol, **more_options):
    ...
    for _ in range(10):
        try:
            response=ham_int.wait_on_response(ham_int.send_command(ASPIRATE96,
...
def dispense_96(ham_int, plate96, vol, **more_options):
    ...
    for _ in range(10):
        try:
            response=ham_int.wait_on_response(ham_int.send_command(DISPENSE96,
                labwarePositions=compound_pos_str_96(plate96),
                dispenseVolume=vol,
                **more_options), raise_first_exception=True)
            logging.info("dispense 96 response")
            logging.info(response)
            print(response)
            break
        except IllegalTargetPlatePositionError:
