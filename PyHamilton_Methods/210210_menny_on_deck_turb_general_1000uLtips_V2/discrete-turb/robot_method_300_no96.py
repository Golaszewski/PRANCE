...
if __name__ == '__main__':
    ...
    with HamiltonInterface(simulate=simulation_on) as ham_int, ClarioStar() as reader_int:
        ...
        std_class = 'StandardVolumeFilter_Water_DispenseSurface_Empty'
        ...
        def service(controllers_for_plate, plate, tips, media_reservoir):
            ...
            for col_num, zip_item in enumerate(control_zip):
                ...
                water_poss_0 = [(water_site0, j) for j in array_idxs] #define bleach location
                aspirate(ham_int, water_poss_0, air_vols,  liquidHeight=air_height, **liq_move_param,) # pre-aspirate air to add extra blowout
                aspirate(ham_int, water_poss_0, wash_vols, liquidHeight=10, **liq_move_param) # aspirate bleach
                dispense(ham_int, water_poss_0, wash_vols, liquidHeight=10, dispenseMode=8, **liq_move_param)
                
                water_poss_1 = [(water_site1, j) for j in array_idxs] #define bleach location
                aspirate(ham_int, water_poss_1, wash_vols, liquidHeight=10, **liq_move_param) # aspirate bleach
                dispense(ham_int, water_poss_1, wash_vols, liquidHeight=10, dispenseMode=8, **liq_move_param)
                
                ethanol_poss = [(ethanol_site, j) for j in array_idxs] #define bleach location
                aspirate(ham_int, ethanol_poss, wash_vols, liquidHeight=10, **liq_move_param) # aspirate bleach
                dispense(ham_int, ethanol_poss, wash_vols, liquidHeight=10, dispenseMode=8, **liq_move_param)
                
                aspirate(ham_int, water_poss_1, wash_vols, liquidHeight=10, **liq_move_param) # aspirate bleach
                dispense(ham_int, water_poss_1, wash_vols, liquidHeight=10, dispenseMode=8, **liq_move_param)
                
                dispense(ham_int, water_poss_1, air_vols, # dispense air volumes twice
                        liquidHeight=30, dispenseMode=9, **liq_move_param)
        ...
        try:
            ...
            while True:
                for turbs_for_plate, controllers_for_plate, plate, tips, media_reservoir in zip(turbs_by_plate, ctrlrs_by_plate, plates, tip_boxes, media_sites):
                    ...
                    reader_protocols = ['kinetic_abs_fast', 'Venus'] #absorbance and luminescence only
                    if no_reader:
                        platedatas = None
                        async_service()
                    elif num_plates>1:
                    ...
                    storage.od_readings = []
                    for i in range(96):
                        slope = 0.55 #2XYT = 0.284, M9 =  0.3 
                        intercept =  0.038#2XYT = 0.055, M9 = 0.03
