...
if __name__ == '__main__':
    ...
    with HamiltonInterface(simulate=simulation_on) as ham_int, ClarioStar() as reader_int:
        ...
        def service(controllers_for_plate, plate, tips, media_reservoir, timestamp):
            logging.info("Logging plate id")
            logging.info(str(plate))
            new_media=False
            ...
            for col_num, zip_item in enumerate(control_zip):
                 ...
                 dispense(ham_int, plate_poss, col_vols, liquidHeight=disp_height, mixCycles=1, mixVolume=mix_vol,
                 ...
                 dispense(ham_int, [(waste_site, j%8 + 88) for j in array_idxs], col_vols, # Dispense into waste:  +88 for far right side of bleach #
                         liquidHeight=80, dispenseMode=8, **liq_move_param)
                 # Rinse in bleach
                 aspirate(ham_int, bleach_poss, wash_vols, **liq_move_param) # aspirate bleach
                 dispense(ham_int, bleach_poss, wash_vols, dispenseMode=8, **liq_move_param)
                 # Blow out air
                 dispense(ham_int, bleach_poss, air_vols, dispenseMode=9, **liq_move_param)
                 tip_eject(ham_int, tip_poss) # put tips back
            ...
            aspirate_96(ham_int, bleach_site, bleach_vol, liquidHeight=30, **liq_move_param) #aspirate bleach
            dispense_96(ham_int, bleach_site, bleach_vol, liquidHeight=30, dispenseMode=8, **liq_move_param)
            dispense_96(ham_int, bleach_site, air_vol, liquidHeight=50, dispenseMode=9, **liq_move_param) #blow out excess air
            
            # Rinse tips in water 3x
            aspirate_96(ham_int, water_site0, air_vol, liquidHeight=air_height, **liq_move_param) #aspirate air
            aspirate_96(ham_int, water_site0, wash_vol, **liq_move_param) #aspirate water
            dispense_96(ham_int, water_site0, wash_vol, dispenseMode=8, **liq_move_param)
            aspirate_96(ham_int, water_site1, wash_vol, **liq_move_param) #aspirate water
            dispense_96(ham_int, water_site1, wash_vol, dispenseMode=8, **liq_move_param)
            aspirate_96(ham_int, ethanol_site, wash_vol, liquidHeight=10, **liq_move_param) #aspirate water
            dispense_96(ham_int, ethanol_site, wash_vol, liquidHeight=10, **liq_move_param)
        ...
        try:
            ...
            while True:
                for turbs_for_plate, controllers_for_plate, plate, tips in zip(turbs_by_plate, ctrlrs_by_plate, plates, tip_boxes):
                    logging.info("logging plate id line 333")
                    logging.info(str(plate))
                    ...
                    if no_reader:
                        ...
                        logging.info("inside correct branch")
                    ...
                    def async_service(args=(controllers_for_plate, plate, tips, media_reservoir, timestamp)):
                        logging.info("Logging plate id line 394")
                        logging.info(args[1])
                        service(*args)
