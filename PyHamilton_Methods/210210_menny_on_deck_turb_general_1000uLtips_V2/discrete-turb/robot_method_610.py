...
from pace_util import (
    ...
    initialize, hepa_on, tip_pick_up, tip_eject, aspirate, dispense, read_plate,  layout_item, wash_empty_washer,
...
media_vols=[[starting_media_vol]*8]*12
    ...
    with HamiltonInterface(simulate=simulation_on) as ham_int, ClarioStar() as reader_int:
        ...
        def service(controllers_for_plate, plate, tips, media_reservoir, timestamp):
            ...
            for col_num, zip_item in enumerate(control_zip):
                 ...
                 subtract_lists=zip(media_vols[col_num],col_vols)
                 media_vols[col_num]=[tracker-transfer for tracker,transfer in subtract_lists]
                 ...
                 logging.info("Media taken from wells"+str(col_vols))
                 logging.info("Media remaining in wells"+str(media_vols[col_num]))
                 
                 if any([media_well<10 for media_well in media_vols[col_num]]) and replace_media==True:
                     assert(num_plates==1)
                     new_media=True
                     logging.info("Moving to next media plate")
            ...
            while True:
                for turbs_for_plate, controllers_for_plate, plate, tips, media_reservoir in zip(turbs_by_plate, ctrlrs_by_plate, plates, tip_boxes, media_sites):
                    ...
                    if num_plates==1:
                        ...
                        new_media=service(*args)
                        if new_media:
                            media_sites=media_sites[1:]
                        media_vols=[[starting_media_vol]*8]*12
                        new_media=False
