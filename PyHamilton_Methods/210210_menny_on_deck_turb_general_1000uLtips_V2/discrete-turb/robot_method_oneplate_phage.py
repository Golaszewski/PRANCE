...
if __name__ == '__main__':
    ...
    waste_washer = layout_item(lmgr, Tip96, 'ht_hw_96washdualchamber2_0001')
    ...
    with HamiltonInterface(simulate=simulation_on) as ham_int, ClarioStar() as reader_int:
        ...
        def service(controllers_for_plate, plate, tips, media_reservoir, timestamp, cycle_washer):
            ...
            dispense_96(ham_int, waste_washer, 500, liquidHeight=5)
            aspirate_96(ham_int, waste_washer, 100, liquidHeight=15) # Aspirate AIR ###NEW
            aspirate_96(ham_int, waste_washer, 600, liquidHeight=5) # rinse from waste
            dispense_96(ham_int, waste_washer, 600, liquidHeight=5, dispenseMode=8) 
            
            aspirate_96(ham_int, water_washer, 800, liquidHeight=5) # quick rinse in water washer 
            dispense_96(ham_int, water_washer, 800, liquidHeight=5, dispenseMode=8) # mode 9 is blowout # mode 4 is jet drain tip
            ...
            for col_num, zip_item in enumerate(control_zip):
                 ...
                 dispense(ham_int, [(waste_washer, j%8 + 88) for j in array_idxs], col_vols, # Dispense into waste:  +88 for far right side of bleach #
                         liquidHeight=5, dispenseMode=8, **liq_move_param)
                 dispense(ham_int, [(waste_washer, j%8 + 88) for j in array_idxs], air_vols, # Dispense into waste:  +88 for far right side of bleach #
                         liquidHeight=5, dispenseMode=9, **liq_move_param)    
                 
                 
                 wash_vols = [bleach_vol for _ in media_poss] # define wash volumes

                 bleach_poss = [(waste_washer, j) for j in array_idxs] #define bleach location
                 ...
                 aspirate(ham_int,water_washer,wash_vols, liquid_height=5)
                 dispense(ham_int,water_washer,wash_vols, liquid_height=5, dispenseMode=8)
                 
                 aspirate(ham_int,water_site0,wash_vols, liquid_height=5)
                 dispense(ham_int,water_site0,wash_vols, liquid_height=5, dispenseMode=8)
        ...
        try:
            ...
            while True:
                for turbs_for_plate, controllers_for_plate, plate, tips in zip(turbs_by_plate, ctrlrs_by_plate, plates, tip_boxes):
                    ...
                    if num_plates==1:
                        args=(controllers_for_plate, plate, tips, media_reservoir, timestamp, rinse_cycle)
                        service(*args)
                        iter_counter+=1
